...
| GitHub, Inc. (`protomaps.github.io`) | Schriften der Vektorkarte | Sieht die IP-Adresse. **Entfällt nicht** durch das eigene Kachelarchiv: Die Glyphen lädt der Kartenstil weiterhin von dort (`map-style.ts`). Das Sprite ist seit dem 7. September raus — FreiFahren hat auch keins —, die Schriften noch nicht; sie selbst auszuliefern steht in `docs/todo.md`. Hier stand vorher, der Abfluss „entfällt" — das war falsch (Audit-Punkt M-017). |
