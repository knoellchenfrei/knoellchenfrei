...
| komoot GmbH (`photon.komoot.io`) | Straßensuche | Seit dem 9. September nachts: Wer im Suchfeld drei oder mehr Zeichen tippt, schickt den getippten Text und den Rahmen der gewählten Stadt an Photon, den freien Geocoder von komoot; dabei sieht komoot die IP-Adresse. Nichts davon wird von uns gespeichert oder mit einer Meldung verknüpft. Unter drei Zeichen und offline bleibt die Suche auf dem Gerät (Zonen und Bezirke). Photon läuft ohne Konto und ohne Schlüssel; die Bedingungen stehen unter <https://photon.komoot.io>. |
