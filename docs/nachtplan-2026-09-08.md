...
| C10 | **Ein Drittel der Hamburger Flächen konnte die falsche Farbe zeigen** | Die Karte färbt über `setFeatureState({ id })`, und `id` kam aus `promoteId: 'zone'`. In Hamburg tragen **44 von 145 Flächen** den Schlüssel `-` — am 9. September nachgemessen: nicht mangels Namen, sondern weil diese Flächen kein Bewohnerparkrecht kennen und die Quelle für sie keine Nummer führt —, dazu vier Zonen in Stücken mit verschiedenen Zeiten (A103: 9–20 und 9–23 Uhr) und eine mit verschiedenen Beträgen. Alle teilten sich einen Zustandsplatz, der letzte Schreibvorgang gewann. Um 21 Uhr stand „frei" über Flächen, die bis 22 Uhr kassieren | Die Zonenschlüssel je Stadt gezählt: 145 Flächen, 63 Schlüssel |
...
| **Und die Reparatur davon war zu grob** | Verloren waren die Überschrift und zwei Punkte; der Rest hing kopflos am Ende des Abschnitts davor. Ich habe aus der fehlenden Überschrift auf einen fehlenden Abschnitt geschlossen und den ganzen wieder eingesetzt — die Hälfte stand danach zweimal da. Aufgefallen erst, als jemand die offenen Punkte aufgelistet haben wollte | Ein fehlender Anker heisst nicht, dass alles dahinter fehlt. Nachzählen, **was** weg ist, statt aus dem Anker zu schliessen — dieselbe Regel wie bei den vier Beispielen, aus denen ich verallgemeinert hatte |
...
| **D1-Migrationen liefen nie** — behoben am 8. September | Der Deploy rief `migrations apply` gar nicht. Seit dem 7. September ruft er es, und bekommt jedes Mal `code: 7403`: Dem CI-Token fehlt `D1:Edit`, es trägt bewusst nur *Workers Scripts:Edit* und *Cloudflare Pages:Edit*. Wegen `continue-on-error` steht darüber eine gelbe Warnung, und der Lauf ist grün | Das Recht hängt seit dem 8. September am Token; der Probelauf meldet `✅ No migrations to apply!`. Gefunden im Log des Laufs von 02:51, **nachdem** ich in `hosting.md` geschrieben hatte, der Schritt tue es — ein Schritt mit `continue-on-error` kann jahrelang scheitern, ohne dass ein Lauf rot wird |
