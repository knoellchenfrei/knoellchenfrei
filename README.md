...
| **Typisch hier** | Im Zonenblatt: wann in dieser Zone typischerweise gemeldet wird — Stufe je Wochenstunde und „in 5 von 16 Dienstagen um 10 Uhr". Aus Zählern je Zone, Wochentag, Stunde und Quartal ohne Datum und Koordinate, zwölf Quartale lang; geglättet über Stadt, Zone und Tagesabschnitt, weil eine einzelne Zelle sonst jahrelang leer bliebe. Sagt „zu wenig Daten", bis es welche gibt. |
...
| **Utrecht, Den Haag, Rotterdam, Groningen, Nijmegen, Eindhoven** | [RDW — Nationaal Parkeer Register](https://opendata.rdw.nl/resource/adw6-9hsg.json), Socrata (acht Tabellen, die Stadt ist ein `areamanagerid`); Stadtteile aus den CBS-Wijken über PDOK | [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/) — gemeinfrei, Namensnennung *optional* | 66 / 88 / 124 / 39 / 29 / 13 Gebiete mit Betaald-parkeren-Regelung als 68 / 93 / 149 / 42 / 31 / 71 Flächen, Tarif als Staffel, Feiertage aus der Quelle — [docs/staedte-niederlande.md](docs/staedte-niederlande.md) |
...
| **Schwerin** | [Landeshauptstadt Schwerin über den Landkreis Ludwigslust-Parchim](https://geoportal.kreis-lup.de/ows/masterportal/parken-sn), WFS 2.0.0 als **GML** in EPSG:25833 | [DL-DE/Namensnennung 2.0](https://www.govdata.de/dl-de/by-2-0) — Quellenvermerk wörtlich `Landeshauptstadt Schwerin` | 10 von 15 Bewohnerparkzonen mit Automaten (die Polygone tragen kein Attribut; die Buchstaben kommen aus der Karte des Dienstes), 143 Parkscheinautomaten als Sachdatenquelle, 64 Behindertenparkplätze, 27 Stadtteile |
| **Graz** | [Magistrat Graz, Stadtvermessungsamt](https://geodaten.graz.at/mapping/rest/services/1_3_Verkehrswesen/Grazer_Parkzonen/FeatureServer), ArcGIS FeatureServer (GeoJSON) | **nicht ausgewiesen** — der Dienst nennt keine Lizenz, die App zeigt das an; Klärung mit dem Stadtvermessungsamt offen. Bezirke aus dem OGD-Dienst unter [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) | 90 Flächen der Blauen Zone (2,60 €/h, höchstens 3 h) und 75 der Grünen Zone (2,00 €/h, Tagesticket), Tarif, Zeiten und Höchstparkdauer je Fläche; 17 Bezirke. 122 der 165 Flächen sind Straßenzüge von 4–7 m Breite — deshalb wie in Karlsruhe der Rückfall auf die nächste Fläche |
...
| **Bern** | [Stadt Bern, Geoportal](https://map.bern.ch/arcgis/rest/services/Geoportal/Parkkartenzonen/MapServer/1), ArcGIS MapServer | [Nutzungsbedingungen betreffend Geodaten der Stadt Bern](https://map.bern.ch/geoportal/data/Nutzungsbedingungen_Geodaten_Stadt-Bern_1.0.pdf) — „Freie Nutzung. Quellenangabe ist Pflicht.", Nennung wörtlich als `Geodaten Stadt Bern` ist **Bedingung** | 34 Flächen für 31 Parkkartenzonen — **ohne Zeiten und ohne Beträge**, die Quelle nennt beides nicht (Klasse C: „Zeiten unbekannt" statt „frei"); 32 statistische Bezirke in 6 Stadtteilen |
| **Krakau** | [Zarząd Transportu Publicznego w Krakowie](https://services-eu1.arcgis.com/svTzSt3AvH7sK6q9/arcgis/rest/services/Granice_Stref_2026/FeatureServer/1), ArcGIS FeatureServer (GeoJSON) | **nicht ausgewiesen** — das Item nennt nur den Vorbehalt „ma charakter poglądowy"; die App zeigt das an, Klärung mit dem ZTP offen | 23 Sektoren des Obszar Płatnego Parkowania (Podstrefa A/B/C, seit dem 10. August 2026 drei neue und ein erweiterter), 18 Dzielnice — **Klasse C**: die Daten nennen weder Zeiten noch Tarif, die App sagt „Zeiten unbekannt" — [docs/staedte-krakau.md](docs/staedte-krakau.md) |
| **Saarbrücken** | [Landeshauptstadt Saarbrücken, Open Data](https://opendata.saarbruecken.de/dataset/parkzonen), statische GeoJSON-Dateien aus dem CKAN der Stadt | **Datenlizenz Deutschland, Variante nicht ausgewiesen** — das Portal nennt nur `datenliz-de`; die App zeigt das an, Klärung mit dem Ordnungsamt offen | 27 Bewohnerparkzonen (A1 … U), deren Buchstaben nur als Beschriftungspunkte in einer zweiten Datei stehen, 20 Stadtteile in vier Stadtbezirken — **Klasse C**: die Daten nennen weder Zeiten noch Tarif, die App sagt „Zeiten unbekannt" — [docs/staedte-saarbruecken.md](docs/staedte-saarbruecken.md) |
| **Gera** | [Stadtverwaltung Gera, Zentrales GIS](https://geoportal.gera.de/geoserver/gera/wfs?service=WFS&request=GetCapabilities), GeoServer-WFS 2.0.0 — die erste Stadt in Thüringen | **nicht ausgewiesen** — `Fees: NONE`, `AccessConstraints: NONE`, kein Katalogeintrag; die App zeigt das an, Klärung mit dem Zentralen GIS offen | 10 Bewohnerparkzonen (A bis L, eine geteilte Fläche C/G) **nur als Grenzen** — keine Zeiten, kein Betrag (Klasse C); die 138 Straßenlinien derselben Ebene stehen als Straßenliste an der Zone, 27 Ortsteile, 12 Behindertenparkplätze — [docs/staedte-gera.md](docs/staedte-gera.md) |
| **Hildesheim** | [Stadt Hildesheim](https://gdi.stadt-hildesheim.de/interface/wfs-ms/Bewohnerparkzonen), MapServer-WFS des Geoportals — GeoJSON kommt in `[lon, lat]`, das GML derselben Ebene gedreht | **widersprüchlich ausgewiesen** — die Nutzungsbedingungen nennen [DL-DE/Namensnennung 2.0](https://www.govdata.de/dl-de/by-2-0) und untersagen zugleich kommerzielle Nutzung ohne Genehmigung; die App zeigt das an, Klärung mit der Stadt offen. Vermerk „Geodaten © Stadt Hildesheim" | 7 Bewohnerparkzonen A–G **nur als Grenzen** — keine Zeiten, kein Betrag (Klasse C: die App sagt „Zeiten unbekannt" statt „frei"); 14 Ortschaften — [docs/staedte-hildesheim.md](docs/staedte-hildesheim.md) |
| **Straßburg** | [Ville de Strasbourg](https://data.strasbourg.eu/api/explore/v2.1/catalog/datasets/stationnement-payant/exports/geojson), Opendatasoft-Export (GeoJSON) — die erste Stadt in Frankreich | [Licence Ouverte / Open Licence 1.0](https://www.etalab.gouv.fr/wp-content/uploads/2014/05/Licence_Ouverte.pdf) — Nennung von Quelle und Stand ist **Bedingung** | 19 Tarifzonen rouge/orange/vert mit **Staffeltarif** am Feature (als Spanne der Stundenkosten, die Staffel wörtlich im Panel) und Höchstparkdauer 3/4/5 h; **Zeiten aus der Datensatzbeschreibung** (Mo–Sa 9–19 Uhr, Feiertage frei — Alsace-Moselle-Kalender `FR-67`); 10 Quartiere — [docs/staedte-strasbourg.md](docs/staedte-strasbourg.md) |
| **Innsbruck** | [Stadt Innsbruck, geoHub](https://services8.arcgis.com/LxSaGwss445axp1E/arcgis/rest/services/Parkzonen_WGS84/FeatureServer/0), ArcGIS FeatureServer | [Nutzungsbedingung der Stadt](https://geohub-1-magibk.hub.arcgis.com/pages/nutzungsbed) — „vergleichbar mit CC BY 4.0", Nennung wörtlich als `Datenquelle: Stadt Innsbruck` ist **Bedingung** | 21 Parkzonen (Kurzparkzonen und Parkstraßen, Zeiten und Tarif aus einem Freitextfeld), 20 Stadtteile; Stand gilt bis zur neuen Parkabgabeverordnung am 2. November 2026 |
| **Zürich** | [Stadt Zürich](https://www.ogd.stadt-zuerich.ch/wfs/geoportal/Gebietseinteilung_Parkierungsgebuehren), WFS 1.1.0 auf QGIS Server — der erste Dienst, der die URN-Form von `srsName` mit 500 quittiert | [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/deed.de) — gemeinfrei, Vermerk „Quelle: Stadt Zürich" empfohlen | 2 Hochtarifzonen (Innenstadt, Oerlikon) mit Bedienungszeit aus dem Feed und **Tarif in Franken aus dem Erlass AS 551.330**, gegengeprüft an 553 der 1.397 Parkuhren, 2.886 gebührenpflichtige Parkfelder darin, 34 Quartiere — [docs/staedte-zuerich.md](docs/staedte-zuerich.md) |
| **Wien** | [Stadt Wien](https://data.wien.gv.at/daten/geo), WFS 2.0.0 | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/deed.de) — Quellenvermerk wörtlich `Datenquelle: Stadt Wien – data.wien.gv.at` | 81 flächendeckende Kurzparkzonen (je Bezirk, als 105 Stücke) und 796 Geschäftsstraßen als 24-m-Bänder, die die Bezirkszone überstimmen; **Tarif aus der Parkometerabgabeverordnung** (3,40 €/h, 15 Minuten gratis), weil der Datensatz keinen Betrag führt; 23 Bezirke — [docs/staedte-wien.md](docs/staedte-wien.md) |
| **Genf** | [SITG — Etat de Genève](https://vector.sitg.ge.ch/arcgis/rest/services/OTC_MACARON/MapServer/0), ArcGIS REST (GeoJSON) — die erste Stadt in der Schweiz | [Conditions d'utilisation des données du Portail SITG](https://sitg.ge.ch/ressources/conditions-utilisation-donnees), Stufe „A – Accès libre (Open Data)" — Quellenangabe `Données SITG` ist **Bedingung** | 17 Macaron-Zonen `A`–`Q` der Ville de Genève (von 53 im Kanton), **ohne Zeiten und ohne Tarif** — die Quelle nennt beides nicht, die App sagt „Zeiten unbekannt"; 19.072 Autoplätze aus 6.213 Stellplatzreihen mit Höchstdauer je Zone, 297 Behindertenparkplätze, 8 Quartiere |
| **St. Gallen** | [Stadt St.Gallen, Open Data](https://daten.stadt.sg.ch/api/explore/v2.1/catalog/datasets/ppv-parkflaeche/exports/geojson), GeoJSON-Export des Opendatasoft-Portals — der erste Dienst, der weder WFS noch ArcGIS ist | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) — Namensnennung ist **Lizenzbedingung**, Portal: „Freie Nutzung mit Quellenangabe" | 2.652 Stellplatzreihen als Stücke zweier Zonen (`EBZ` 1.871, `Parkuhr` 781) — **die Quelle kennt keine Zonen, nur Parkfelder mit Markierungsart**, keine Zeiten, kein Betrag (Klasse C: „Zeiten unbekannt"); 114 Behindertenparkplätze, 31 statistische Quartiere — [docs/staedte-stgallen.md](docs/staedte-stgallen.md) |
| **Kassel** | [Stadt Kassel, Vermessung und Geoinformation](https://geoportal.kassel.de/arcgis/rest/services/Service_Daten/Verkehr_Mobilitaet/MapServer/27), ArcGIS MapServer — die erste Ebene, die über `query` keine Geometrie herausgibt; der Abruf ist ein `identify` über den Gemeindeumriss | **nicht ausgewiesen** — der Dienst nennt keine Lizenz, die App zeigt das an; Klärung mit dem Amt für Vermessung und Geoinformation offen. Ortsbezirke unter [DL-DE/Namensnennung 2.0](https://www.govdata.de/dl-de/by-2-0) | 29 Bewohnerparkbezirke **nur als Grenzen** — keine Zeiten, kein Betrag (Klasse C: die App sagt „Zeiten unbekannt" statt „frei"); 24 Ortsbezirke — [docs/staedte-kassel.md](docs/staedte-kassel.md) |
| **Essen** | [Stadt Essen](https://opendata.essen.de/sites/default/files/Bewohnerparkbereiche.geojson), GeoJSON-Dateien aus dem DKAN-Portal — kein WFS | [DL-DE/Namensnennung 2.0](https://www.govdata.de/dl-de/by-2-0) — Namensnennung ist **Lizenzbedingung** | 9 Bewohnerparkbereiche **ohne Zeiten und Tarif** (Klasse C: die App zeigt die Grenze und sagt „Zeiten unbekannt"; die sechs Parkzonen der Stadt gibt es nicht als Geometrie), 50 Stadtteile, Umweltzone — [docs/staedte-essen.md](docs/staedte-essen.md) |
...
| | |
| --- | --- |
| Unit-Tests | 2400 — 1828 in `core`, 101 für den Worker (gegen echtes SQLite) und das Zählwerk, 337 für die Web-App, 134 für Artifact-Bau, Datenstand, Abzeichen und Einheiten. Ein grosser Teil davon sind Regressionstests für konkrete gefundene Fehler; die 83 Regeln in `CLAUDE.md` sind die nachzählbare Seite davon |
