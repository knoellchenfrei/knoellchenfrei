...
describe('representativePoint liegt in seiner eigenen Fläche', () => {
  ...
  it('findet die Daten aller acht Städte', () => {
    expect(STAEDTE).toEqual(['berlin', 'duesseldorf', 'frankfurt', 'graz', 'hamburg', 'karlsruhe', 'koeln', 'muenchen'])
    const summe = STAEDTE.reduce((n, stadt) => n + flaechen(stadt).length, 0)
    // Am 16. September 890 (Berlin 103, Düsseldorf 44, Frankfurt 27, Graz 165,
    // Hamburg 145, Karlsruhe 279, Köln 45, München 82).
    expect(summe).toBeGreaterThanOrEqual(840)
  })
