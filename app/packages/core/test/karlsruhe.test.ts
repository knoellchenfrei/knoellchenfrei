...
describe('eine Karlsruher Fläche im gemeinsamen Tarifmodell', () => {
  ...
  /**
   * Der Stolperdraht für Baden-Württemberg.
   *
   * Solange `Land` das Kürzel nicht kennt, wirft `holidaysFor` — und das ist
   * richtig so: Eine Stadt ohne Feiertagskalender forderte an Karfreitag zum
   * Zahlen auf und sähe dabei nach nichts aus. Dieser Test fällt in dem
   * Moment, in dem jemand `BW` in `REGIONAL` einträgt; genau dann soll er
   * gelesen und der Eintrag oben gegen die Quelle gehalten werden.
   *
   * Der Cast ist der einzige in dieser Datei und steht hier, weil die Aussage
   * gerade lautet: Dieses Kürzel ist zur Übersetzungszeit **kein** `Land`.
   */
