...
describe('Innsbruck als Stadt', () => {
  ...
  it('weist Hall in Tirol ab und überlässt Salzburg und München ihren Städten', () => {
    expect(cityAt(11.5086, 47.2814)).toBeUndefined() // Hall in Tirol
    // Salzburg ist seit demselben Tag selbst angeschlossen — der Punkt gehört
    // ihm, nicht Innsbruck und nicht niemandem.
    expect(cityAt(13.0433, 47.8095)?.key).toBe('salzburg')
