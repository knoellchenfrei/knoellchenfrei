...
describe('Feiertage', () => {
  /**
   * Der Zustand ist heute **laut**, und das ist Absicht: Eine Zone mit
   * `land: 'NW'` bricht ab, statt an Fronleichnam still zum Zahlen
   * aufzufordern. Dieser Test gehört ersetzt, sobald der Eintrag
   * `NW: { fixed: ['11-01'], fromEaster: [60] }` in `holidays.ts` steht —
   * der Wortlaut aus § 2 Abs. 1 des Feiertagsgesetzes NW liegt vor und steht
   * in `docs/staedte-duesseldorf.md`.
   */
  it('kennt Nordrhein-Westfalen noch nicht, und sagt das laut', () => {
    // Der Cast ist nötig und begründet: `Land` kennt `NW` noch nicht, und
    // genau das hält dieser Test fest.
    expect(() => holidaysFor('NW' as Land, 2026)).toThrow(/NW/u)
  })
})
