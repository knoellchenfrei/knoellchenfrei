...
describe('eine Kölner Zone in der Zeit', () => {
  ...
  /**
   * Der Feiertagskalender, den es noch nicht gibt.
   *
   * `holidaysFor` wirft bei einem unbekannten Land, statt eine leere Menge zu
   * liefern — und das ist hier genau richtig: Eine Kölner Zone mit
   * `land: 'NW'` würde heute laut abbrechen statt an Fronleichnam still zum
   * Zahlen aufzufordern. Der Test hält den Zustand fest; er fällt, sobald
   * jemand `NW` einträgt, und genau dann gehört er ersetzt.
   *
   * Der Cast ist begründet: `Land` kennt `NW` nicht, und genau das wird hier
   * geprüft — die Prüfung greift zur Laufzeit, weil `land` aus Konfiguration
   * kommt und damit aus einer Datei, die der Übersetzer nie gesehen hat.
   */
  it('hat noch keinen Feiertagskalender: Nordrhein-Westfalen fehlt in holidays.ts', () => {
    expect(() => holidaysFor('NW' as Land, 2026)).toThrow(/NW/)
  })
})
